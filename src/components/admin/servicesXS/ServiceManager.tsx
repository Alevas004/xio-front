...
import ModalUpdateServiceXS, { ServiceUpdate } from "./ModalUpdateServiceXS";
import ModalCreateServiceNew, { ServiceCreate } from "./ModalCreateServiceNew";
...
const ServiceManager = () => {
  const [selectedService, setSelectedService] = useState<ServiceUpdate | null>(
    null
  );
  ...
  const { data, error, loading, refetch } = useGet<GetData>(
    `/xiomarasanchezterapeuta/servicesxs${
  ...
  } = useUpdate<ServiceUpdate, Service>("xiomarasanchezterapeuta/servicesxs", {
    withAuth: true,
  });
  ...
  const {
    ...
    error: errorPost,
    loading: loadingPost,
  } = usePost<ServiceCreate>("/xiomarasanchezterapeuta/servicesxs/create", {
    withAuth: true,
  });
  ...
  const handleInfoUpdate = (service: ServiceUpdate) => {
    setSelectedService(service);
    setEditModal(true);
  };

  const onEditService = async (body: ServiceUpdate) => {
    if (!selectedService) return;

    console.log(
      `Edit service with id: ${selectedService.id}`,
      "info service for edit:",
      body
    );
    const updateData = {
      ...body,
    ...
    await updateItem(selectedService.id, updateData);
    await refetch();
    setEditModal(false);
  };

  //   //* CREATE SERVICE
  const onCreateService = async (body: ServiceCreate) => {
    await create(body);
    await refetch();
  ...
  useEffect(() => {
    if (deleteData) {
      console.log("Servicio eliminado exitosamente, refrescando lista...");
      refetch();
    }
  }, [deleteData, refetch]);
            ...
            <div className="space-y-2 sm:space-y-3 flex flex-col justify-center">
              ...
              <div>
                <p className="text-sm sm:text-lg text-white font-medium">
                  Administra los servicios de terapia de Xiomara Sánchez
                </p>
              </div>
            ...
            <div className="flex justify-center items-center">
              ...
              >
                <div className="flex items-center justify-center space-x-3">
                  ...
                  <span className="text-sm">Crear Nuevo Servicio</span>
                </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
              ...
              <div className="flex-1 sm:flex-none">
                <select
                  ...
                  className="w-fit sm:w-auto px-3 py-2 bg-white/20 border border-white/30 rounded-lg text-white text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-white/50 focus:bg-white/30 transition-all"
                >
                  <option value="all" className="bg-purple-600 text-white">
                    Todas las categorías
                  </option>
                  <option
                    value="masaje-prenatal"
                    className="bg-purple-600 text-white"
                  >
                    🤰 Masaje Prenatal
                  </option>
                  <option
                    value="masaje-terapeutico"
                    className="bg-purple-600 text-white"
                  >
