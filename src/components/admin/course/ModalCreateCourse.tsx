...
const createSchema = z.object({
  ...
  tags: z.array(z.string().min(1, { message: "El tag no puede estar vacío" })),
  price: z
    .number()
    .nonnegative({ message: "El precio debe ser mayor o igual a 0" }),
  is_free: z.boolean(),
  level: z.enum(["beginner", "intermediate", "advanced"]),
  duration: z.number().positive({ message: "La duración debe ser mayor a 0" }),
  ...
  is_active: z.boolean(),
  academyId: z.string().uuid({ message: "academyId inválido" }).optional(),
...
interface CreateProps {
  onClose: () => void;
  onSave: (data: CourseCreate) => Promise<void>;
  error: string | null;
  loading: boolean;
}
...
const ModalCreateCourse = ({
  ...
  const [belongsToWorkshop, setBelongsToWorkshop] = useState<boolean>(false);
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-sm text-white/90">
                Agrega un curso educativo profesional
              </p>
            </div>
          </div>
          <button
          ...
          <div className="border-b bg-green-50 px-6 py-4">
            ...
            {loading && (
              <div className="flex items-center space-x-3 text-green-600">
                ...
                <span className="text-sm font-medium">Creando curso...</span>
              </div>
            )}
          </div>
        )}
          ...
          <form
            onSubmit={handleSubmit((data) => {
              const courseData: CourseCreate = {
                ...
                belongsToAWorkshop: data.belongsToAWorkshop,
              };
              return onSave(courseData);
            })}
            className="p-6"
          >
